...
.. code-block:: ABNF
   ...
   flag-sign       = %x80          ; message payload is a signed token
...
When the ``flag-sign`` flag is set, the message payload frame SHALL contain
a NULL-terminated token generated by :func:`flux_sign_wrap` over the actual
message payload content. The token has the form ``HEADER.PAYLOAD.SIGNATURE``
as described by the flux-security API, where HEADER encodes the userid and
signing mechanism, PAYLOAD is the base64-encoded payload content, and
SIGNATURE is mechanism-dependent.

``flag-payload`` SHALL be set alongside ``flag-sign``, since the token
occupies the payload frame.

Upon receipt of a message with ``flag-sign`` set, the receiver SHALL:

#. Call :func:`flux_sign_unwrap` to verify the signature and extract the
   signing userid and payload content.

#. Reject the message if signature verification fails.

#. Reject the message if the signing userid does not match the message
   userid set at ingress by the connector.

#. Treat the extracted payload content as the authoritative message payload.

Since the signing userid is verified to match the message userid, it is
accessible via the standard message userid accessor.

.. note::

   ``flag-sign`` consumes the final available bit in the flags octet.
   Future protocol extensions requiring additional flags would require a
