...
* **task** A single process that is part of a Flux program. Each task within
  a program has a unique identifier called the rank.
...
* **task slot** One or more resources which run a single executable task.  
 ...
 * **shrink** Remove resource set R' from program P. Tasks currently
   executing on task slots removed from P SHALL be terminated. 
   _shrink (P, R')_. If _R' == R_ then P becomes empty again and SHALL
   have no running tasks.
