...
specified either by the user in *_J_* or by system configuration, but
...
the *job shell path* specified in *_J_*, or a system default. After the
call to exec(2) the security IMP is replaced by the guest user process,
